{
    ...
    init: function(elevators, floors) {
        ...
        function isAnybodyElseGoingThere(floorNum, distance, direction){
            ...
            var flag11 = false;
            _.each(elevators, function(elevator){
                elevator.checkDestinationQueue();//update
        ...
        function normal(){
            ...
            _.each(elevators, function(elevator){
                ...
                var pressed = checkInside(elevator);
                if((upFloorCallList.length > 0) || (downFloorCallList.length > 0) || (pressed.length > 0)){ //POSSIBLE
                    ...
                    if(checkInside(elevator).length > 0){
                    ...
                    if(direction === "up"){
                        ...
                        for(var i = 0; i < upFloorCallList.length; i+= 1){//check to see if there are any pressed floors(outside) in the direction we are going but ahead of us...list is presorted
                            if(upFloorCallList[i] > currentFloor){//add all the current                                
                                ...
                                for(var k = i; k <upFloorCallList.length; k+= 1){
                                    upFloorCallList.splice(k,k+1);//removes aas they are added to dest
                                }//close for
                        ...
                        for(var i = (downFloorCallList.length-1); i > 0; i--){//check to see if there are any pressed floors(outside) in the direction we are going but ahead of us...list is presorted
                            if(downFloorCallList[i] < currentFloor){//add all the current
                                for(var j = i; j > 0; j--){
                                    ...
                                    downFloorCallList.splice(k,k+1);//removes as they are added to dest
                                    k--;
                                }//close for
                                for(var k = i; k > 0; k--){
                                    upFloorCallList.splice(k,k+1);//removes aas they are added to dest
                ...
                }else{//nobody is doing anything...let's get organized
        ...
        //CHECKPOINT B
        //this is now good
        function bsFloors(master, type, i){//binary search of the floors against pending list
            console.log("bsFloors");
            var left = 0;
            var right = master.length - 1;
            var mid = Math.round((left + right) / 2)//find midpoint
            master = master.sort();
            var tempList = type > 0 ? TupFloorCallList : TdownFloorCallList;

            //boundary catch
            if((master[left] > tempList[i]) || (master[right] < tempList[i])){
                master.push(tempList[i]);//added it to the master
                type > 0 ? TupFloorCallList.splice(i, i+1) : TdownFloorCallList.splice(i, i+1);
            ...
            while(true){
                ...
                if(left > right){
                    ...
                    type > 0 ? TupFloorCallList.splice(i, i+1) : TdownFloorCallList.splice(i, i+1);
                    return master;
                }else if(master[mid-k] === tempList[i]){
                    type > 0 ? TupFloorCallList.splice(i, i+1) : TdownFloorCallList.splice(i, i+1);
        ...
        var TupFloorCallList = [];//temp storage for floor lists
        var TdownFloorCallList = [];//temp storage for floor lists
        ...
        function checkFloors(){
            ...
            _.each(floors, function(floor){
                ...
                floor.on("up_button_pressed", function(){
                    ...
                    TupFloorCallList.push(floor.level);   
                    // console.log("floor press up " + floor.level);
                });
                
                floor.on("down_button_pressed", function() {
                    //processUpCallListTemp(0);
                    TdownFloorCallList.push(floor.level);
                    // console.log("floor press down " + floor.level);
                });
            });
            //fixed this ^^ kept erasing the TupFloorCallList
            if(upFloorCallList.length != numFloors){
                // console.log("inside A");
                //console.log("upCallListTemp " + TupFloorCallList.length);
                //check each floor in temp against master list

                for(var i = 0; i < TupFloorCallList.length; i++){
                    // console.log("inside A-1-1");
                    upFloorCallList = bsFloors(upFloorCallList, 1, i);
                }
            }//close if
            ...
            if(downFloorCallList.length != numFloors){
                // console.log("inside B");
                  //check each floor in temp against master list
                for(var i = 0; i < TdownFloorCallList.length; i++){
                    // console.log("inside A-1-2");
                    console.log(downFloorCallList.length + " ");
                    downFloorCallList = bsFloors(downFloorCallList, -1, i);
                }
            }//close if
            console.time('normal');
        ...
        _.each(elevators, function(elevator){
            elevator.on("idle", function() {
                ...
                console.time("checkFloors");
                checkFloors();
                console.timeEnd("checkFloors");
            });
        });
